...
class _HomeShellState extends State<HomeShell> {
        ...
        builder: (context, snapshot) {
          ...
          return FutureBuilder<RegistrationStatus>(
            ...
            builder: (context, statusSnap) {
              final status =
                  statusSnap.data ?? RegistrationStatus.notRegistered;
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          Row(
            children: [
              ClipRRect(
                ...
                child: Image.asset(
                  'lib/auth/screen/asset/logo.png',
                  width: 32,
                  height: 32,
                  fit: BoxFit.cover,
                ),
          ...
          const Text(
            'Selamat datang kembali di panel monitoring Anda.',
          ...
          Row(
            children: [
              _statCard(
                ...
                label: 'LOGBOOK',
                value: '${data.logbookCount}',
                detail: 'Hari ini / total',
                tint: const Color(0xFFEAE8FF),
                onTap: () => accepted ? _go('/logbook') : _showLocked(),
              ),
              const SizedBox(width: 12),
              _statCard(
                icon: Icons.check_circle_outline,
                label: 'PRESENSI',
                value: data.hasPresensiToday ? '1' : '0',
                detail: 'Kehadiran',
                tint: const Color(0xFFD7F9E9),
                onTap: () => accepted ? _go('/presensi') : _showLocked(),
              ),
              const SizedBox(width: 12),
              _statCard(
                ...
                value: '0',
                detail: 'Diterbitkan',
                tint: const Color(0xFFFFEAD5),
                onTap: () =>
                    accepted ? _go('/nilai-sertifikat') : _showLocked(),
              ),
            ],
          ),
          const SizedBox(height: 26),
          ...
          _activity(
            ...
            title: 'Presensi',
            subtitle: 'Catat kehadiran harian Anda',
            color: const Color(0xFFEAE8FF),
            action: Icons.login_rounded,
            onTap: () => accepted ? _go('/presensi') : _showLocked(),
          ),
          ...
          _activity(
            ...
            title: 'Input Logbook',
            subtitle: 'Lengkapi aktivitas harian Anda',
  ...
  void _showLocked() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'Pendaftaran Anda belum disetujui. Fitur ini akan tersedia setelah admin menerima pendaftaran Anda.',
        ),
  ...
  Widget _error() => Center(
    child: Column(
      ...
      children: [
        const Text('Dashboard belum dapat dimuat.'),
      ...
      children: [
        Row(
          children: [
            ...
            Text(
              '${progress.percent}%',
              style: const TextStyle(fontSize: 23, color: _primary),
            ),
          ],
        ),
        ...
        ClipRRect(
          ...
          child: LinearProgressIndicator(
            ...
            minHeight: 9,
            backgroundColor: const Color(0xFFE8EDFA),
            color: _primary,
          ),
        ),
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Minggu ${progress.week} dari ${progress.totalWeeks}',
                  style: const TextStyle(
                    fontSize: 10,
              ...
              const Icon(Icons.arrow_forward, size: 15, color: _primary),
            ],
          ),
        ),
      ],
        ...
        child: Row(
          children: [
            ...
            Container(
              ...
              child: Icon(action, color: Colors.white, size: 18),
            ),
          ],
        ),
      ),
...
class _InternshipProgress {
  ...
  factory _InternshipProgress.fromDates(
    DateTime? start,
    DateTime? end,
    DateTime now,
  ) {
    if (start == null || end == null || end.isBefore(start)) {
      return const _InternshipProgress(0, 0, 0, 0, 0);
    }
    final total = end.difference(start).inDays + 1;
    final passed = now.isBefore(start)
        ? 0
        : now.isAfter(end)
        ? total
